...
from app.models import CanonicalAsset, CanonicalAssetCreate, CanonicalAssetPublic, CanonicalAssetUpdate, CanonicalAssetAlias, CanonicalAssetAliasPublic, CanonicalAssetAliasCreate
...
def read_assets(
    ...
    alias_query = select(CanonicalAssetAlias).where(CanonicalAssetAlias.asset_uid.in_(asset_uids))
    aliases = session.exec(alias_query).all()
    
    alias_map = {uid: [] for uid in asset_uids}
    for a in aliases:
